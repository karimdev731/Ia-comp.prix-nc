  ...
  const processExtractedItems = async (items: string[]) => {
    ...
    try {
      ...
      if (result.searchResults.some(products => products.length > 0)) {
        try {
          const recommendationsResult = await LangChainService.generateShoppingRecommendations(
          ...
          if (cartItems.length > 0) {
            ...
            const analysisResult = await LangChainService.analyzeShoppingPatterns(mockPurchaseHistory);
